...
<div class="flex flex-col items-center justify-center mt-4 px-4">
    ...
    <button type="button"
        ...
        Version: <strong class="ml-1 text-purple-300">{{session('version', '2.0')}}</strong>
    ...
    <div
        ...
        <div class="flex flex-col items-center w-full">
            @foreach(\App\Services\Documentation::versionLinks() as $link)
                <a
                    href="{{ url($link['url']) }}"
                    class=" py-2 font-medium transition flex flex-row items-center justify-center w-full bg-gray-700 text-white text-sm hover:bg-gray-900 hover:text-purple-400 hover:font-bold">
                    {{$link['title']}}
                </a>
            @endforeach
